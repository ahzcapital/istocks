...
export class ConfiguredEgyptHttpProvider {
  ...
  async fetch(): Promise<ProviderPayload> {
    if (!this.url) {
      throw new Error('EGX_MARKET_DATA_URL is not configured.');
    }
    ...
    if (!response.ok) {
      throw new Error(`EGX market-data provider returned HTTP ${response.status}.`);
    }

    const payload: unknown = await response.json();
    if (!isRecord(payload) || !Array.isArray(payload.observations) || !isRecord(payload.fx)) {
      throw new Error('EGX market-data provider returned an unsupported response shape.');
    }
    ...
    for (const item of payload.observations) {
      ...
      observations.push({
        ...
        source: asString(item.source) ?? 'configured-egx-http-provider',
        currency: asString(item.currency) ?? 'EGP',
        countryCode: asString(item.countryCode) ?? 'EG',
      });
    }

    const fx: FXObservation = {
      baseCurrency: asString(payload.fx.baseCurrency) ?? 'USD',
      quoteCurrency: asString(payload.fx.quoteCurrency) ?? 'EGP',
