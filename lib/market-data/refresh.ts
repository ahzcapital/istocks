...
export async function refreshEgyptMarketData(now = new Date()) {
  ...
  const providerConfigured = liveProvider.isConfigured();
  ...
  if (providerConfigured) {
    ...
    try {
      ...
      return { ok: false, market: 'EG', providerMode, providerConfigured, error: error instanceof Error ? error.message : 'Live EGX provider failed.', persisted: false, persistenceReason: 'No fallback to the static snapshot is performed after a configured live-provider failure.' };
    }
  } else {
  ...
  if (!validation.ok || fxIssues.length > 0) return {
    ok: false, market: 'EG', providerMode, providerConfigured, observations: validation,
    fx: { ok: false, observation: fx, issues: fxIssues }, persisted: false,
    persistenceReason: 'Validation failed; no market data was persisted.',
  };

  if (!providerConfigured) return {
    ok: true, market: 'EG', providerMode: 'snapshot', providerConfigured: false, observations: validation,
    fx: { ok: true, observation: fx, issues: [] }, persisted: false,
    persistenceReason: 'EGX_MARKET_DATA_URL is not configured; static snapshot was validated but not persisted as live data.',
  };

  try {
    const persistence = await persistEgyptMarketData({ observations, fx, marketStatus, timestamp: now });
    return {
      ok: persistence.persisted, market: 'EG', providerMode, providerConfigured, observations: validation,
      fx: { ok: true, observation: fx, issues: [] }, persisted: persistence.persisted,
