...
describe('GCC market registry',()=>{
  ...
  it('keeps GCC market caps internally consistent with USD conversion',()=>{for(const code of Object.keys(FX) as Array<keyof typeof FX>){const rows=getMarketCompaniesSync(code);expect(rows.length).toBeGreaterThan(0);for(const row of rows){const expected=(row.marketCapLocal??0)/FX[code];expect(Math.abs(row.marketCapUSD-expected)).toBeLessThan(100)}}});
});
