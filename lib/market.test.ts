...
describe('GCC market registry',()=>{
  ...
  it('keeps GCC market caps internally consistent with USD conversion',()=>{for(const code of ['SA','AE','KW','QA','BH','OM']){const rows=getMarketCompaniesSync(code);expect(rows.length).toBeGreaterThan(0);for(const row of rows)expect(row.marketCapUSD).toBeCloseTo((row.marketCapLocal??0)/({SA:3.75,AE:3.6725,KW:0.3065,QA:3.64,BH:0.376,OM:0.3845}[code as keyof typeof {SA:3.75,AE:3.6725,KW:0.3065,QA:3.64,BH:0.376,OM:0.3845}]),-2)}});
});
