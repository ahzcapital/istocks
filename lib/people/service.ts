...
export async function rankPeople(params:{q?:string;country?:string;category?:string;period?:string;sort?:string}) {
  ...
  let people = PUBLIC_PEOPLE.filter(person => {
  ...
  const rows = people.map(person => {
    ...
    return {...person,likes:s.likes,dislikes:s.dislikes,netLikes:s.likes - s.dislikes,rank:0,userVote:'none' as const};
  });
  ...
  rows.sort((a,b) => {
    ...
    if (sort === 'country') return a.countries[0].localeCompare(b.countries[0]) || a.name.localeCompare(b.name);
